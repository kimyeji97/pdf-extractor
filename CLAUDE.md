...
2026-07-26 기준 각 prefix 다음 번호: `B10`, **`C09`**(C08=문제집 삭제, 2026-07-25 구현), **`D08`**(D07=템플릿 리디자인, 진행 중), `F09`, `P04`.
(2026-07-21 기준은 `B10`, `C08`, `D07`, `F09`, `P04` 이었음)
...
### 0-6. 2026-07-25~26 버그 6건 + REQ-D07 프론트 리디자인 착수

**테스트 발견 버그·개선 6건 (커밋 `a778803`)** — `docs/예정된작업.md` 2026.07.25 목록 중 서버 여백 2건·로그인/공유 제외 전부:
- 생성 이력→편집 복원 시 이미지 없음: `editor/index.jsx`가 `thumbnailUrl: null` 하드코딩. 썸네일 URL은 결정적이라 조립하면 됨. 같은 자리에서 `questionId`도 복합키로 바로잡음(번호만 쓰면 멀티 파일 문제집에서 키 충돌).
- ① 문항 수에 수동 문항 누락: `list_pages`의 `question_count`는 `questions_per_page`(자동 감지분)에서만 옴 → 전체 문항 일괄 API로 페이지별 재집계. 같은 데이터로 **오탐 페이지 하이라이트**도 구현.
- 이름/유형 편집을 문제집 편집 → 문항 분석 목록으로 이동.
- **삭제 기능 2종 신설**: `DELETE /api/jobs/{id}`, `DELETE /api/workbooks/{id}`. 사용자 결정에 따라 **연관 저장물 전부 삭제**(원본·결과·상태·경계·썸네일·수동문항·페이지캐시). R2는 `delete_objects` 1,000개 배치. 소스를 지우면 그 문항으로 만든 이력은 남지만 편집 화면 이미지가 안 보인다 — 다이얼로그에 명시.
- **문항별 배율 조절**: `SelectionItem.scale` 신설. PDF는 `show_pdf_page`에 대상 클리핑이 없어 **넘치는 만큼 원본 clip을 줄이는** 방식으로 구현(벡터 유지 + 이웃 셀 침범 없음). 연타 시 stale 값이 쓰이던 버그를 updater 전달로 수정.

**REQ-D07 리디자인 — Phase 1·2·3-1·3-2·3-3 완료**. 상세는 `docs/specs/20260725-REQ-D07-minimal-template-adoption.md` §4-1.
...
**작업 중 알게 된 것**
- **높이 체인이 이식의 핵심 적응 지점**: 템플릿은 body가 스크롤되는 문서형 전제. 이 앱은 100dvh 작업대라 root→sidebarContainer→main 전 구간에 `flex:1`/`minHeight:0`/`overflow:hidden`을 걸어야 한다. 이게 REQ-B04·B05·B08 계약의 뿌리다.
- **우리 코드에도 죽은 파일이 있었다**: `QuestionPicker`(624줄)·`SelectionBasket`(366줄)·`StatusPoller`·`QuestionInput`·`NavMenu`. 도달성 분석(진입점부터 import 추적)으로 확인 후 제거 — Phase 3 마이그레이션 대상에서 ~1,100줄이 빠졌다.
- **`WorkbookPreview`의 색은 토큰화하지 않는다**: UI 색이 아니라 생성될 PDF 지면을 재현한 값이고 `pdf_service`의 라벨 배경과 짝을 이룬다. `PAPER` 상수로 이름만 붙였다.
- **책등 색은 유형이 아니라 이름 해시**: 실데이터의 `workbook_types`가 자유 입력이라 5권에 13종이고 고정 분류가 없었다.
- MUI 7.0(템플릿) → 7.3(우리)에서 `shape.borderRadius` 타입이 `number | string`으로 넓어짐.

**남은 것**: Phase 3-4(문항 분석 작업 — 뷰어 좌표·높이 체인 계약 집중), 3-5(문제집 편집 868줄 + 멀티 파일 선택 노출), Phase 4(추가 기능).
