...
**남은 작업**: REQ-P02(클라 성능 — 뷰어 가상화·getPages 낭비 제거·dedup 등), REQ-P03(서버 성능 — 썸네일 6~10초 병목·캐시·페이지네이션). 2026-07-22 P02/P03를 클라/서버로 재구성 완료(스펙 확정, 착수 전).
...
| 11 | 서버(BE) | 서버 성능: 썸네일 6~10초 병목(전체 PDF 재read) + 캐시·페이지네이션·비동기 | REQ-P03 | 성능 | ⬜ 예정 |
...
[Phase 6] ⬜ 예정 — 성능 (2026-07-22 재구성: P02=클라 / P03=서버)
  REQ-P03 (서버: 썸네일 6~10초 병목=전체 PDF 재read, 프로파일링 우선 → 캐시·페이지네이션·비동기)
  REQ-P02 (클라: 뷰어 가상화·getPages 낭비 제거·dedup·리스트 memo)
