...
- 진행 중: **REQ-D07** 프론트 전면 리디자인 — Phase 1~4 완료(스펙 §4-2).
  ...
  **Phase 0~3 완료 2026-08-27**(dev 실측 통과: 전송 0.3~1.3s, 숨김 탭 즉시, 폴링 0건). 브랜치
  `feat/P04-sse-push`는 PR #2로 **main 머지 완료(2026-08-27, `d176596`)**. 파생 **REQ-C09**(알림 경로 후속 4건 — 실패 문구·kind 필터·딤 케이스·`: connected` 선발송) Phase 1·2 완료 2026-08-28, `feat/C09-notification-followups` 미머지. 파생 **REQ-B11**(새로고침 토스트 — 기준선을 빈 목록에서 잡던 버그) Phase 1~2 완료 2026-08-28(dev 프론트 Worker에 배포됨 — 새로고침 5회 토스트 0건 확인), `feat/B11-notification-baseline` PR #3로 **main 머지 완료(2026-08-28, `3d35d65`)**.
