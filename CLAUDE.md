...
- **현재 위치 (2026-09-13)**: **TODO 로드맵 3단계 종료** — 신규 항목 5개(D11·B12·F12·REQ-29·
  REQ-30)가 전부 main에 들어왔다(PR #9·#10·#11·#12·#13). 다음은 **테마 재정의**(번호 보류 —
  REQ-30이 생겼으므로 템플릿 속성으로 흡수할지 이제 판단할 수 있다. `/workplan`으로 착수) →
  **REQ-27 로그인**
...
- ⚠️ **REQ-29·REQ-30에 같은 잔여 위험 2건이 겹쳐 쌓여 있다** — ①**s3 스토리지 경로 미검증**
  (계약 #24 격리로 로컬 테스트는 항상 local 백엔드라 구문 검사만 했다. 표지·각주·워터마크에
  이어 **템플릿 스토리지도** 같은 상태) ②**브라우저 end-to-end 미실시**(선택 →
  `startExtractV2` 인자 → 요청 body → PDF 반영을 고리별로만 검증. 생성·편집·템플릿 관리 화면은
  렌더 무대가 없어 소스 스캔까지만 덮인다). **둘 다 다음 dev 프론트 재배포에서 처음 실제로 도는
  경로다** — 그 배포에는 지금 **아홉 건이 밀려 있으니**(아래 배포 상태) 한 번에 올라간다.
  배포하면 여기부터 확인할 것.
...
(2026-08-28 배포 정책 — 변경은 모아서 한 번에), C09·D09·F10·D10·D11·B12·F12·REQ-29·REQ-30 프론트 변경이
