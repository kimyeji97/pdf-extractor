...
**남은 작업**:
- **REQ-P03 (서버, 진행 중)** — 다음 착수: **P03-01 썸네일 프리워밍(전 페이지)**. 개별 썸네일 GET(`get_thumbnail`, `get_question_thumbnail_endpoint`)이 아직 캐시 미스 시 전체 PDF 재다운로드 → 업로드/감지 시점에 이미 받은 `pdf_bytes`로 전 페이지 썸네일+문항 크롭을 미리 렌더·캐시(마진 비용은 렌더뿐, 206p×~30ms≈6초 백그라운드). 이후 P03-05(단일 요청 중복 read)·P03-03(페이지네이션)·P03-04(비동기).
- **REQ-P02 (클라)** — 미착수. P02-02(목록 카드 `getPages` 제거, P03-01과 함께 적용해야 목록 로딩 근본 개선)·P02-01(뷰어 가상화, F07 오버레이 좌표 계약 유지) 우선.
...
| 11 | 서버(BE) | 서버 성능: 썸네일 6~10초 병목(전체 PDF 재read) + 캐시·페이지네이션·비동기 | REQ-P03 | 성능 | 🔄 진행중 (P03-02 완료, 다음 P03-01) |
...
- **REQ-P03 (서버 성능)**: 2026-07-22 재구성 후 **Backend 전용**(구 P02 백엔드 7개 이관 + 썸네일 병목). **원인 규명 완료**: 6~10초 = 캐시 미스 시 전체 PDF 재다운로드(R2 왕복) + 전체 문서 파싱이 썸네일 1장당 반복. P03-01(재read 제거+프리워밍)·P03-02(페이지 메타 캐시)가 핵심. P02-02와 함께 적용해야 목록 로딩 근본 개선.
...
[Phase 6] 🔄 진행중 — 성능 (2026-07-22 재구성: P02=클라 / P03=서버)
  REQ-P03 (서버) 진행중:
    ✅ P03-01 프로파일링 (R2 다운로드 ~99% 병목 확정)
    ✅ P03-02 페이지 메타 캐시 (list_pages 2.75s→0.17s)
    ⬜ 다음: P03-01 썸네일 프리워밍(전 페이지) → P03-05 → P03-03 → P03-04
  REQ-P02 (클라) 미착수: P02-02(getPages 제거) → P02-01(뷰어 가상화) 등
  ※ P02-02(목록 카드 getPages 제거) ↔ P03-01(전체 PDF 재read 제거)은 함께 적용해야 목록 로딩 근본 개선
