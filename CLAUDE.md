...
- **현재 위치 (2026-09-13)**: TODO 3단계 신규 항목 **5개 전부 완료**(D11·B12·F12·REQ-29·REQ-30).
  다음은 **테마 재정의**(번호 보류 — REQ-30이 생겼으므로 템플릿 속성으로 흡수할지 이제
  판단할 수 있다. `/workplan`으로 착수) → **REQ-27 로그인**
...
- ⚠️ **REQ-29·REQ-30에 같은 잔여 위험 2건이 겹쳐 쌓여 있다** — ①**s3 스토리지 경로 미검증**
  ...
  렌더 무대가 없어 소스 스캔까지만 덮인다). **둘 다 다음 dev 배포에서 처음 실제로 도는 경로다** —
  배포하면 여기부터 확인할 것.
