...
2026-07-04 기준 각 prefix 다음 번호: `B05`, `C07`, `D06`, `F07`, `P03`.

## 예정 작업 계획 (2026-07-04 스냅샷)

`docs/예정된작업.md`를 기반으로 계획 번호를 부여하고 사전 논의 사항·작업 순서를 정리한다. (아직 spec 미작성 항목 다수 — 아래는 작업계획서 작성 **전** 단계 메모)

### 1. 작업 목록 & 계획 번호

| # | 영역 | 작업 | 계획번호 | 유형 |
|---|------|------|----------|------|
| 1 | 문항 분석 | PDF 뷰어 툴바 버그: 이전/다음 버튼 무동작 + 툴바가 스크롤 영역에 포함됨 | REQ-B05 | 버그 |
| 2 | 문항 분석 | 문항 목록 벌크(다중) 삭제 안됨 | REQ-B06 | 버그 |
| 3 | 문항 분석 | 페이지 미리보기를 PDF 뷰어로 전환 (생성 이력 REQ-F06 방식 재사용) | REQ-F07 | 개선 |
| 4 | 문항 분석 | 오탐 의심 문항 체크박스 비활성 → 삭제 불가 해소 | REQ-B07 | 버그 |
| 5 | 문제집 편집 | 문항 선택 목록 스크롤 문제 (REQ-B04와 동일 원인) | REQ-B08 | 버그 |
| 6 | 문제집 편집 | 문항 라벨 포맷 변경 (`{번호}.{문제집} p{페이지}` → `+ {문항 이름}`) | REQ-C07 | 보완 |
| 7 | 문제집 편집 | 다운로드 PDF에서 문항 라벨 `...` 축약 방지 | REQ-B09 | 버그 |
| 8 | 문제집 편집 | 우측 미리보기 스크롤 방향 좌우 → 상하 | REQ-F08 | 개선 |
| 9 | 표지 관리 | 목록형 1패널 + 업로드 모달로 재설계 (기존 REQ-D05 방향 상충) | REQ-D06 | 디자인 |
| 10 | 전체 | 속도·품질 성능 개선 (뷰어 가상화, 캐시, 페이지네이션, 비동기 등) | REQ-P02 | 성능 |
| 11 | 전체 | 썸네일 요청 응답 6~10초 → 1초 이하 단축 | REQ-P03 | 성능 |
...
```
[Phase 1] 독립 소규모 버그 (선착수, 의존성 없음)
  REQ-B08 (편집 스크롤 — B04 패턴 재사용)
  REQ-F08 (미리보기 스크롤 방향)
...
[Phase 3] 라벨 묶음 (동시 진행)
  REQ-C07 (라벨 포맷)  +  REQ-B09 (축약 방지)
  ※ 라벨 길이 증가와 축약 로직이 한 셀 안에서 충돌하므로 함께 설계

[Phase 4] PDF 뷰어 묶음 (REQ-F06 기반)
