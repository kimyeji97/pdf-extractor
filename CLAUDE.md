...
- **현재 위치 (2026-09-13)**: **TODO 로드맵 3단계 종료** — 신규 항목 5개(D11·B12·F12·REQ-29·
  REQ-30)가 전부 main에 들어왔다(PR #9·#10·#11·#12·#13). 이어서 **REQ-F13**(미리보기에 템플릿
  반영 + 워터마크 알파 결함)도 main 에 들어왔다(PR #14). **하루에 REQ 세 개가 사슬로 닫혔다** —
  REQ-30 이 만든 기능이 화면에 안 보인다는 질문이 F13 을 낳고, F13 의 미리보기가 REQ-29 의
  워터마크 결함을 드러냈다. **테마 항목은 2026-09-13에 폐기**(❌ — 사유 "차후 고려해서
  작업해도 문제 없음. 지금 필요없는 기능". 두 번 미뤄지는 동안 **"무엇을 어디에 적용하는지"가
  끝내 정의되지 않았다.** TODO 에 항목·갈래 셋·재개 자산을 남겨 뒀다).
  이어서 **REQ-B13**(문항 크롭 여백 네 변 10pt 통일) Phase 1 완료 — **Phase 2(오탐 판정 실측)만
  남았다.** 그 뒤 **REQ-27 로그인**
...
- ⚠️ **REQ-29·30·F13의 잔여 위험** — ①**s3 스토리지 경로**: 자동 테스트는 계약 #24 격리로 항상
  ...
  **미검증으로 남는 것은 ECS 배포 경로뿐이다.** ②**브라우저 end-to-end**: 생성·편집·템플릿 관리
  ...
  테스트가 됐다). 다음 dev 프론트 재배포에는 **열 건이 밀려 있다**(아래 배포 상태).
...
(2026-08-28 배포 정책 — 변경은 모아서 한 번에), C09·D09·F10·D10·D11·B12·F12·REQ-29·REQ-30·F13 프론트 변경이
