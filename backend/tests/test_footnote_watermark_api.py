...
def test_29_01_각주_등록시_201과_footnote_id(authed_client):
    """근거: PLAN § 범위 — "텍스트를 저장한다는 점만 빼면 표지 CRUD(`cover.py`)와 같은 모양" """
    res = authed_client.post("/api/footnotes", json={"name": "저작권 문구", "text": "본 자료의 무단 배포를 금합니다."})

    assert res.status_code == 201
    assert res.json()["footnote_id"]


def test_29_02_등록한_각주가_목록에_나타난다(authed_client):
    """근거: PLAN § 범위 — "텍스트를 저장한다는 점만 빼면 표지 CRUD(`cover.py`)와 같은 모양" """
    created = authed_client.post("/api/footnotes", json={"name": "저작권 문구", "text": "내용"}).json()

    res = authed_client.get("/api/footnotes")

    ids = [f["footnote_id"] for f in res.json()["footnotes"]]
    assert created["footnote_id"] in ids


def test_29_03_각주_삭제후_목록에서_빠진다(authed_client):
    """근거: PLAN § 범위 — "텍스트를 저장한다는 점만 빼면 표지 CRUD(`cover.py`)와 같은 모양" """
    created = authed_client.post("/api/footnotes", json={"name": "저작권 문구", "text": "내용"}).json()

    authed_client.delete(f"/api/footnotes/{created['footnote_id']}")

    ids = [f["footnote_id"] for f in authed_client.get("/api/footnotes").json()["footnotes"]]
    assert created["footnote_id"] not in ids


def test_29_04_없는_각주_삭제는_404(authed_client):
    """근거: PLAN § 범위 — cover.py 기존 패턴(존재하지 않는 id 삭제 시 404)"""
    res = authed_client.delete("/api/footnotes/no-such-id")
...
def test_29_05_워터마크_등록시_201과_watermark_id(authed_client):
...
def test_29_06_등록한_워터마크가_목록에_나타난다(authed_client):
    """근거: PLAN § 범위 — "워터마크 CRUD(`/api/watermarks`) — 표지와 동일하게 이미지 업로드·목록·삭제" """
    ...
    res = authed_client.get("/api/watermarks")

    ids = [w["watermark_id"] for w in res.json()["watermarks"]]
    assert created["watermark_id"] in ids


def test_29_07_워터마크_이미지_조회시_바이트를_반환한다(authed_client):
    """근거: PLAN § 범위 — "워터마크 CRUD(`/api/watermarks`) — 표지와 동일하게 이미지 업로드·목록·삭제" """
    created = authed_client.post(
        "/api/watermarks",
        files={"file": ("wm.png", _png_bytes(), "image/png")},
    ...
    res = authed_client.get(f"/api/watermarks/{created['watermark_id']}/image")

    assert res.status_code == 200
    assert len(res.content) > 0


def test_29_08_워터마크_삭제후_목록에서_빠진다(authed_client):
    """근거: PLAN § 범위 — "워터마크 CRUD(`/api/watermarks`) — 표지와 동일하게 이미지 업로드·목록·삭제" """
    created = authed_client.post(
        "/api/watermarks",
        files={"file": ("wm.png", _png_bytes(), "image/png")},
    ...
    authed_client.delete(f"/api/watermarks/{created['watermark_id']}")

    ids = [w["watermark_id"] for w in authed_client.get("/api/watermarks").json()["watermarks"]]
    assert created["watermark_id"] not in ids


def test_29_09_허용안된_형식_업로드는_400(authed_client):
...
def test_29_10_10MB_초과_업로드는_400(authed_client):
    """근거: PLAN § 결정 — "표지와 동일 — JPEG/PNG만, 최대 10MB" """
