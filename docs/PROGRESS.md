...
| REQ-B10 | 생성 중 화면 이탈 시 문제집 메타 유실 | [plan](plans/PLAN-B10-workbook-meta-lost-on-navigate.md) | — | 🟡 Phase 1~3 완료(수정 끝), 고아 3건 처리 대기 |
| REQ-F09 | 문항 분석·문제집 생성 완료 알림 | [plan](plans/PLAN-F09-completion-notification.md) | — | 🟡 계획 작성, B10 선행 |
...
**남은 것은 고아 3건 처리(Phase 4)뿐이다.** 사용자가 **내용을 기억하지 못해 복구는 기각**했다 —
