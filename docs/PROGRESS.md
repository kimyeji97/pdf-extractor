...
| REQ | 기능명 | 출처 | 규모 | 상태 |
|-----|--------|------|------|:----:|
| REQ-29 | 각주·워터마크 등록 + 생성 PDF 반영 (표지 CRUD와 같은 모양) | [TODO 신규 ③](TODO.md) | 중 (백엔드+프론트) | ❌ 미착수 — REQ-30 선행. 각주 글자는 계약 #10 |
| REQ-30 | 템플릿 — 표지·각주·워터마크 조합 엔티티 + 생성 화면에서 `cover_id` 대신 `template_id` 선택 | [TODO 신규 ④](TODO.md) | 중~대 | ❌ 미착수 — **REQ-29 선행** |
