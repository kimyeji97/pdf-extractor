...
| REQ-D09 | 생성 이력 화면 구조 개편 (목록 전체 폭 카드 그리드 + 클릭 시 우측 아코디언 미리보기 720px) | [plan](plans/PLAN-D09-F10-history-screen-rework.md) | — | 🟡 **Phase 1~4 완료**(케이스 14/14 · `/testrun` 확인) · Phase 5(dev 백엔드로 육안 검증) 대기 |
| REQ-F10 | 생성 이력 이름 검색 | [plan](plans/PLAN-D09-F10-history-screen-rework.md) | — | 🟡 **Phase 3 완료**(D09-08~12, 이 REQ의 전체 범위) · Phase 5 육안 검증(뒷페이지 검색 확인 포함) 대기 |
...
> 2026-09-01에 Phase 1~4가 완료되어 이 표에서 빠졌다**(위 인덱스 🟡). F10의 "서버 검색 선행
