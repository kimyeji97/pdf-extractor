...
| REQ-F09 | 문항 분석·문제집 생성 완료 알림 | [plan](plans/PLAN-F09-completion-notification.md) | — | 🟡 v1 코드 완료(케이스 47/47) — **육안 검증 진행 중**(첫 회에 벨 뱃지 결함 1건 수정) · 미배포 · Phase 6 이연 |
...
| REQ-P04 | 상시 폴링 → 웹소켓 전환 (알림 피드·감지·생성) | 2026-08-03 F09 미결 검토 | 중~대 — 백엔드 커넥션 관리 신설 | ❌ 미착수 |
| REQ-F11 | 재감지 중 상세 진입 차단 (모달 → 목록) · 대기 중이면 '재감지' 버튼만 차단 | 2026-08-10 F09 육안 검증 | 소 — 프론트만. **F09 이전부터 있던 구멍**(URL 직접 진입·뒤로가기) | ❌ 미착수 |
