...
| REQ-D11 | 메뉴 이름 변경(분석·생성·결과·템플릿 관리) + 경로 변경(`/create`·`/results`·`/templates`, 리다이렉트 없음) | [plan](plans/PLAN-D11-menu-rename.md) | — | 🟡 **계획서 + 검증 계약 7건(Phase 1) 작성 2026-09-03, 미결 0건.** 구현 미착수 — 다음은 `/implement D11 1`. 테스트 2파일은 미커밋(`/implement`가 코드와 함께 커밋) |
...
| REQ | 기능명 | 출처 | 규모 | 상태 |
|-----|--------|------|------|:----:|
| REQ-B12 | 목록을 거치지 않는 작업 화면 진입 경로 — 벨 클릭 시 문서 이름이 job-id로 뜸 + URL 직접 진입이 목록으로 돌아감(D10 Phase 2 관찰) | [TODO 신규 ⑤](TODO.md) | 소 (프론트) | ❌ 미착수 — D11 다음. 두 증상이 같은 원인인지는 착수 시 확인(추정) |
| REQ-F12 | 문항분석 현황판 — 오탐·수동 개수(클릭 시 페이지 리스트 → 이동) + 분석 정확도 | [TODO 신규 ⑥](TODO.md) | 중 (프론트 위주) | ❌ 미착수 — **어느 화면인지 미결**(목록 `StatCards` vs 작업 화면 헤더), 통계 API 필요 여부 미결 |
