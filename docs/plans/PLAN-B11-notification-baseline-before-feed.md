...
> 출처: 2026-08-27 세션(P04 Phase 3 브라우저 실측 중 사용자 발견) · 작성: 2026-08-27 · 상태: ✅ 완료 (Phase 1 2026-08-27 · Phase 2 2026-08-28, PR #3 main 머지 2026-08-28 `3d35d65` · 미결 0건)
...
- [x] **`ready`가 켜지는 같은 렌더(커밋)에 신규 알림이 함께 실려 오면 기준선인가 신규인가.** → **기준선으로 확정 — 현재 동작 명문화**(2026-08-28 결정). 확률 낮고, 놓쳐도 벨 배지·팝오버에는 남는다. "신규"로 하려면 GET 응답 집합을 따로 노출해야 해 표면이 늘어난다. 제약("effect로 미루지 않는다")은
