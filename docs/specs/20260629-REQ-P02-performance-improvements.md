...
#### P02-10. StrictMode 이중 요청 = dev 전용 착시 (문서화)  *(신규 — 클라 성능 분석 Q1)*
...
React `<StrictMode>`가 개발 모드에서 effect를 의도적으로 2회 실행(cleanup 정합성 검증)하기 때문이다. **프로덕션 빌드(`npm run build`)에서는 이중 실행하지 않아 배포 환경에선 발생하지 않는다.** → **최적화 대상 아님.** 실질 중복은 P02-03(dedup)에서 처리.

> StrictMode를 끄면 dev에서 이중 호출은 사라지지만, effect 부작용 버그 조기 발견 이점을 잃으므로 **유지 권장**. 본 항목은 "성능 문제로 오인 금지"를 명시하는 문서화 목적.
